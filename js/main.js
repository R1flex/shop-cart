...
document.querySelector('button.add_new').addEventListener('click', function(e) {
    ...
    if(name && price && count) {
        ...
        Swal.fire({
            icon: "error",
            title: "Ошибка",
            text: "Поажалуйста заполните все поля!",
        })
    }
})
    ...
    if(goods.length) {
        ...
        for(let i=0; i<goods.length; i++) {
            ...
            <tr class="align-middle">
                ...
                <td>${goods[i][3]}</td>
                <td><button class="good_delete btn-danger text-bg-danger" data-delete=${goods[i][0]}>&#10006;</button></td>
                <td><button class="good_delete btn-primary text-bg-primary" data-goods=${goods[i][0]}>&#10149;</button></td>
            </tr>
            `
            )
            if(goods[i][4]>0) {
                ...
                <tr class="align-middle">
                    ...
                    <td class="price_discount"><input data-goodid="${goods[i][0]}" type="text" value="${goods[i][5]}" mim="0" max"100"></input></td>
                    <td>${goods[i][6]}</td>
...
document.querySelector('.cart').addEventListener('input', function(e) {
    ...
    let goods= JSON.parse(localStorage.getItem('goods'))
    for (let i=0; i<goods.length; i++) {
        if(goods[i][4]>0 && goods[i][0] == e.target.dataset.goodid) {
